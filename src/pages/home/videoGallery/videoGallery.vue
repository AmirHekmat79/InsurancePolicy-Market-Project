<template>
  <div
    id="videoGallery"
    v-if="videoGalleries.length"
    class="videoes-gallery-section flex column justify-center q-mr-md items-center q-py-xl"
  >
    ...
    <div class="row videoes-gallery-container">
      <div v-if="!showMore" class="col-12 flex justify-center items-center">
        <div
          @click="openDialog(item.metaMediaFileUrl, item.title)"
          class="q-pa-md relative-position"
          v-for="item in videoGalleries.slice(0, 4)"
          :key="item.id"
        >
          ...
          <div class="cursor-pointer">
            <q-img
              ...
              src="images/play-button-round-icon.svg"
            ></q-img>
          </div>
        </div>
      </div>
  ...
  <q-dialog class="Dialog" v-model="showDialog" :full-width="$q.screen.lt.lg">
    <q-card :class="{ 'lg-width': $q.screen.gt.md }">
      <q-card-actions align="right" class="bg-white text-white">
        <q-badge
          flat
          class="popup-btn cursor-pointer q-pa-sm rounded-borders bg-red-5"
          v-close-popup
        >
          بستن</q-badge
        >
