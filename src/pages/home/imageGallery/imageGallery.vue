<template>
  ...
  >
    <div class="col-12 flex justify-center items-center title-container">
    ...
    >
      ...
      <div v-else class="col-12 flex justify-center items-center">
        <div
          class="flip-box"
          v-for="(item, index) in imageAlbums.slice(0, 4)"
          :key="index"
    ...
    <div
      v-if="imageAlbums.length > 4"
      class="col-12 show-more-button-container flex justify-center items-center"
    >
      <a
        class="show-more-button flex justify-center items-center"
        rounded
    ...
    <q-dialog dir="rtl" v-model="showGalleryDetailsDialog">
      <q-card class="gallery-details-dialog" v-if="showGalleryDetailsDialog">
        <q-card-section
        ...
        <q-card-section>
          <div class="flex justify-center items-center">
            <q-img
              class="gallery-details-image"
              width="200px"
              v-for="(item, index) in galleryDetails"
              :key="index"
              :src="item.metaMediaFileUrl"
              @click="magnifyAlbum(item)"
            />
          </div>
        </q-card-section>
      </q-card>
  ...
  <q-dialog
    v-model="showGalleryInnerDetails"
    :full-width="$q.screen.lt.lg"
    id="dialog"
  >
    <q-card
      class="res-card cursor-pointer"
      :class="{ 'lg-width': $q.screen.gt.md }"
    >
      <q-card-section>
        <q-img
...
export default defineComponent({
  ...
  data() {
    return {
      imageAlbums: [],
      baseData: [],
      showGalleryDetailsDialog: false,
      showGalleryInnerDetails: false,
      galleryDetails: [],
      selectedImageSrc: "",
      selectedImageTitle: "",
      showMore: false,
  ...
  mounted() {
    ...
    this.imageAlbums = this.baseData.imageAlbums;
  },

  methods: {
    handleAlbum(item) {
      this.showGalleryDetailsDialog = true;
      this.galleryDetails = item.galleryDetails;
      this.selectedImageTitle = item.title;
      console.log(this.selectedImageTitle);
    },
    magnifyAlbum(item) {
      this.showGalleryInnerDetails = true;
      this.selectedImageSrc = item.metaMediaFileUrl;
      this.selectedImageTitle = item.title;
    },
    handleShowMore() {
